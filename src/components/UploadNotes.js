...
export default function UploadNotes() {
  ...
  const uploadNotes = async () => {
    ...
    formData.append("branchFolderName", branch);
    formData.append("semesterFolderName", semester);
    formData.append("subjectFolderName", subject);
    if (file) {
      formData.append("Files", file);
    }

    const response = await fetch("https://notevista.onrender.com/api/v1/upload", {
      method: "POST",
  ...
  return (
    <div>
      ...
      {isLoading === false ? <div className="btn">
        <button onClick={uploadNotes}>Upload notes</button>
      </div> : <div className="uploaded">
      <ClipLoader color="#123abc" loading={isLoading} size={50} />
      </div>}

      {uploadSuccess && <div className="uploaded">
        <h2>Hurray! Notes Uploaded</h2>
      </div>}
    </div>
  );
}
